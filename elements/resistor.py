...
class Resistor(Element):
    def __init__(self, number: int):
        ...
        self.resistance = self.spawn_param_field('Сопротивление', value=1000,
                                                 key=f'res{self.number}')
        self.acad_text = ''

    def make_rectangle_b_more(self, ro_square: float,
                              mat_power: float, x_pos: float) -> float:
        """
    ...
    def make_rectangle_l_more(self, ro_square: float,
                              mat_power: float, x_pos: float) -> float:
        """
    ...
    def make_meandr(self, ro_square: float,
                    mat_power: float, x_pos: float) -> float:
        """
