...
class CapacitorCounter:
    def __init__(self, number: int):
        ...
        self.material_table = Material('materials/'
                                       'diel_materials.json').condensator_df()
        self.kz = st.sidebar.number_input('Укажите коэффициент запаса',
    ...
    def choose_material(self) -> list:
        ...
        return list(self.material_table.loc
                    [st.selectbox('Укажите материал:',
                                  self.material_table.index.values)])
    ...
    def calc_errors(self, material: list, temperature: int):
        ...
        self.work_power = st.slider('Укажите рабочее напряжение',
                                    min_value=material[2],
                                    max_value=material[3],
                                    value=material[2])
        error_ys = st.sidebar.selectbox('Укажите погрешность '
